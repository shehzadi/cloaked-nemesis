...
int logic_drop(int *test_vec, int node_val)
{
    ...
    for(j=0; j<5; j++)
	{
		...
		{
			if(i != node_val)
				eval_drop(i);	//to avoid overwriting data anywhere after initialization
		}
        
		for(i = 0; i < Npo; i++)
		{
			if(Poutput[i]->val == 2 || Poutput[i]->val == 3 )	// Check if it is D or B
			{
	...
	else if (gname(sortedNodes[xp].type)=="AND")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			...
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					temp1 = AND_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[i+1]->num));
				}
	...
	else if (gname(sortedNodes[xp].type)=="OR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					temp1 = OR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[i+1]->num));
				}
			}
	...
	else if (gname(sortedNodes[xp].type)=="NAND")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					temp1 = NAND_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[i+1]->num));
				}
			}
			sortedNodes[xp].drp_val = temp1;
		}
	...
	else if (gname(sortedNodes[xp].type)=="NOR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					temp1 = NOR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[i+1]->num));
				}
			}
			sortedNodes[xp].drp_val = temp1;
		}
	...
	else if (gname(sortedNodes[xp].type)=="XOR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					temp1 = XOR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[i+1]->num));
				}
			}
			sortedNodes[xp].drp_val = temp1;
		}
