...
typedef struct n_struc {
   ...
   char *vectors[2];	/* Used to store the vector generated by ATPG */ //Akash //Working
   // Both are not bit accessible
   //Akash //making trials for dynamic memory allocation // Chk FLT_COL for its malloc
} NSTRUC;

/*----------------- Command definitions ----------------------------------*/
#define NUMFUNCS 8	//Akash
int cread(), pc(), help(), quit(), lev(), SimpleLogicSimulation(), fault_collapsing(), fault_dropping();	//Akash
// fault_dropping( int *) // Akash // If needed to pass arrary. Check if need to pass this earlier
struct cmdstruc command[NUMFUNCS] = {
   {"READ", cread, EXEC},
   {"PC", pc, CKTLD},
   {"HELP", help, EXEC},
   ...
   {"FLT_COL", fault_collapsing, CKTLD},	//Akash
...
fault_dropping(char *vec_test) // - In Process // Akash
{
	...
	for(i == Nnodes-1; i >=0; i--)
	{
		for(flag = 0; flag <2; flag ++) // had to make loop to save 
		{
			...
			{
				//fault excitation(i, flag);	// we excite the fault location and type of fault
				//pass = logic simulation(vec_test); // want to simulate the logic
				// i want the logic simulation to actually give me pass or fail
				if(pass == 1)
				{
					Node[i].fault[flag] = '0';
					printf("The vector was able to detect this fault");
					fprintf(flt_vec,"%d %s \n", Node[i].num, vec_test);
				}
			}
			//else
