...
int cread(), pc(), help(), quit(), lev(), /*SimpleLogicSimulation(),*/ fault_collapsing(), fault_dropping(), atpg(), logic_drop();	//Akash
...
fault_drop_col(int flt_indx) // - DONE
{
	...
	switch(Node[flt_indx].type)
	{
		...
		//BRCH and XOR does not have a dominance relationship
		case NOT:
				// As both output faults have one different pair. So input remain, output removed!!!
				Node[flt_indx].fault[0] = '0';	//Equivalence Relation // correct
				Node[flt_indx].fault[1] = '0';	//Equivalence Relation // correct
				break;
		case XOR:
...
fault_dropping(int *vec_test) // - In Process // Akash
{
	...
	for(i == Nnodes-1; i >= 0; i--)
	{
		...
		{
			if(sortedNodes[i].fault[flag] == '1')	// 1 says that the fault is active and not yet removed
			{
				// Initialize done in the drop function
				// Initialize_fault(i, flag);	// we excite the fault location and type of fault
				//if(flag)	//SA1
					//sortedNodes[i].drp_val = 3; //B
				//else		//SA0
					//sortedNodes[i].drp_val = 2; //D
					
				//pass = logic_drop(vec_test, i, flag); // want to simulate the logic
...
//int logic_drop()//int *test_vec, int node_val, int flag)
int logic_drop()
{
	int ret_val = 0;				// fail == 0 ***** pass == 1
	int i,j;
	
	if(Gstate != CKTLVL && Gstate != CKTCOL)	//Akash
	{
		lev(NULL);
		Gstate = CKTLVL;
	}
int value_eval;
//=========================TEST=========================================	
int flag;	// 0 - SA0 // 1 - SA1
int node_val;
int test_vec[Nnodes];


		printf("\nEnter the vector of size %d :   ", Npi);
		...
		for(i = 0; i < Npi; i++)
		{
			if(i != node_val)	//to avoid overwriting data on PI after initialization
			{
				...
				sortedNodes[i].drp_val = test_vec[i];
			}
		}

		printf("\nEnter the node and the fault:  ");
		scanf("%d %d", &node_val, &flag);

			// if(flag)	//SA1
				// sortedNodes[node_val].drp_val = 3; //B // correct
			// else		//SA0
				// sortedNodes[node_val].drp_val = 2; //D // correct
...
//==================================================================
		...
		{
			printf("checking now : %d \t", i);
			if(i != node_val)	//for normal values
			{	
				sortedNodes[i].drp_val = eval_drop(i);
				printf("  No Site  \t ");
			}
			else	// to check if fault can be excited or not
			{
				printf("Fault Site \t ");
				value_eval = eval_drop(i);	//to avoid overwriting data anywhere after initialization
				
				if(flag && !value_eval)	//SA1	// 0 no fault / 1 fault
					sortedNodes[node_val].drp_val = 3; //B
				else if(!flag && value_eval)	//SA0	// 0 no fault / 1 fault
					sortedNodes[node_val].drp_val = 2; //D
				else
					return 0;
			}
			printf("Value at the node: %d\n", sortedNodes[i].drp_val);
		}
        printf("\n\n");
		
		ret_val = 0;
				
		for(i = Nnodes - 1; i >= Nnodes - Npo; i--)
		{
			printf("checking outputs : %d : %d\n", i, sortedNodes[i].drp_val);
			
			if(sortedNodes[i].drp_val == 2 || sortedNodes[i].drp_val == 3 )	// Check if it is D or B
			{
				printf("\n\nFault Dropped");
				ret_val = 1;
				break;
...
int eval_drop(int xp)
{
	int p, temp1, temp2;
	 ...
	 if(gname(sortedNodes[xp].type)=="PI")
	 	{
        //printf("adding value &d to node %d\n", input_vector[sortedNodes[xp].num-1], sortedNodes[xp].num);
		// sortedNodes[xp].drp_val = input_vector[input_vector_pointer];
        // input_vector_pointer++;//input vector is a 5 location array that has values 1, 0 or -1(aka X)
			return input_vector[xp];
		}
	
	if (gname(sortedNodes[xp].type)=="BRANCH")	// Does not matter how many branches it goes to
	{
		//sortedNodes[xp].drp_val = getNodeVal(sortedNodes[xp].unodes[0]->num);
		return(getNodeVal(sortedNodes[xp].unodes[0]->num));
	}
	...
	else if(gname(sortedNodes[xp].type)=="NOT")
	{
		// if(getNodeVal(sortedNodes[xp].unodes[0]->num)==0)
		// {
            // sortedNodes[xp].drp_val = 1;
        // }
		// else if(getNodeVal(sortedNodes[xp].unodes[0]->num)==1)
		// {
            // sortedNodes[xp].drp_val = 0;
        // }
		// else if(getNodeVal(sortedNodes[xp].unodes[0]->num)==2)
		// {
            // sortedNodes[xp].drp_val = 3;
        // }
		// else if(getNodeVal(sortedNodes[xp].unodes[0]->num)==3)
		// {
            // sortedNodes[xp].drp_val = 2;
        // }
		
		//sortedNodes[xp].drp_val = NOT_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num));
		return (NOT_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num)));
	...
	else if (gname(sortedNodes[xp].type)=="AND")
	{
		...
		{
			//printf("AND with more than 1 input\n");
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				if(p == 0)
				{
					temp1 = AND_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num));
				}
				else
				{
					temp1 = AND_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[p+1]->num));
				}

			}
            //sortedNodes[xp].drp_val = temp1;
			return temp1;
        }
		else
		{
            ...
            //sortedNodes[xp].drp_val = getNodeVal(sortedNodes[xp].unodes[0]->num);
			return (getNodeVal(sortedNodes[xp].unodes[0]->num));
            // printf("Assigned value %d to node %d because upnode[0] has value. Upnode node is %d out of %d nodes\n", sortedNodes[xp].drp_val, sortedNodes[xp].num, sortedNodes[xp].unodes[xp]->num, sortedNodes[xp].fin);
        }
	}
	...
	else if (gname(sortedNodes[xp].type)=="OR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				...
				{
					//  printf("calling OR function for the first time for node %d with inputs %d and %d\n", sortedNodes[xp].num,sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num);
					temp1 = OR_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num));
				}
				else
				{
					temp1 = OR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[p+1]->num));
				}
			}
	...
	else if (gname(sortedNodes[xp].type)=="NAND")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				if(p == 0)
				{
					temp1 = NAND_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num));
				}
				else
				{
					temp1 = NAND_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[p+1]->num));
				}
			}
			//sortedNodes[xp].drp_val = temp1;
			return temp1;
		}
		else	// Check for one Input gate
		{
			//sortedNodes[xp].drp_val = getNodeVal(sortedNodes[xp].unodes[0]->num);
			//sortedNodes[xp].drp_val = NOT_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num));	//Acts as Invertor
			return (NOT_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num)));	//Acts as Invertor
		}
	}
	...
	else if (gname(sortedNodes[xp].type)=="NOR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				if(p == 0)
				{
					temp1 = NOR_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num));
				}
				else
				{
					temp1 = NOR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[p+1]->num));
				}
			}
			//sortedNodes[xp].drp_val = temp1;
			return temp1;
	...
	else if (gname(sortedNodes[xp].type)=="XOR")
	{
		...
		if(sortedNodes[xp].fin>1)
		{
			for(p = 0; p <= sortedNodes[xp].fin-2; p++)
			{
				if(p == 0)
				{
					temp1 = XOR_4TT(getNodeVal(sortedNodes[xp].unodes[0]->num), getNodeVal(sortedNodes[xp].unodes[1]->num));
				}
				else
				{
					temp1 = XOR_4TT(temp1, getNodeVal(sortedNodes[xp].unodes[p+1]->num));
				}
			}
			//sortedNodes[xp].drp_val = temp1;
			return temp1;
		}
		else	// Check for one Input gate. Should be 0
		{
			//sortedNodes[xp].drp_val = getNodeVal(sortedNodes[xp].unodes[0]->num);
			//sortedNodes[xp].drp_val = 0;	//Both input same so output 0
			return 0;
		}
	}	
}
