...
/*----------------- Command definitions ----------------------------------*/
#define NUMFUNCS 6
int cread(), pc(), help(), quit(), lev(), SimpleLogicSimulation();
struct cmdstruc command[NUMFUNCS] = {
   {"READ", cread, EXEC},
   {"PC", pc, CKTLD},
   {"HELP", help, EXEC},
...
//this just returns V*. Still need to calculate intersection to update and figure out conflict or success
void forwardImplication(enum e_gtype gateType, int A[], int B[])
{
    FILE *f_tt1;
    char *array_6t;
    char rd1, rd2, rd3;
    ...
    //there is already a global variable array to store what values are possible global_pos[4]
    //if AND gate open AND gate 3 valued truth table
        f_tt1 = fopen("AND","r");
        for(i=0; i<4; i++){
            ...
            while((fscanf(f_tt1,"%c %c %c", &rd1, &rd2, &rd3))!= EOF)
                {
                        if(equate(rd3)==i){
                            ...
                            if(((A[equate(rd1)]==1) || (equate(rd1)==4))&&(((B[equate(rd2)])==1) || (equate(rd2)==4))){
                                global_pos[i] = 1;
                            }
...
void backwardImplication(enum e_gtype gateType, int *A, int *B, int &changedA, int &changedB, int C)
{
    FILE *f_tt1;
    char *array_6t;
    int InputPoss[4][4];
    ...
    //there is already a global variable array to store what values are possible global_pos[4]
    //if AND gate open AND gate 3 valued truth table
    f_tt1 = fopen(array_6tt,"r");
        //for(i=0; i<4; i++){
           
    while((fscanf(f_tt1,"%c %c %c", &rd1, &rd2, &rd3))!= EOF)
        {
                if(equate(rd3)==C){
                    //Take input A is the controller first
                    for(j=0; j<4; j++){

                        if((equate(rd1)==4)||((equate(rd1)==j)&&(A[j]==1))){
                            for(k=0; k<4; k++){

                                if((equate(rd2)==4)||((equate(rd2)==k)&&(B[k]==1))){
                                        InputPoss[j][k] = 1;
                    ...
                    //sum results from controller As processing
                    for(j=0; j<4; j++){
                        for(k=0; k<4; k++){
                            if(InputPoss[j][k]==1){
                                newA[k] = 1;
                    ...
                    //Take input B as the controller
                    for(j=0; j<4; j++){

                        if((equate(rd2)==4)||((equate(rd2)==j)&&(B[j]==1))){
                            for(k=0; k<4; k++){

                                if((equate(rd1)==4)||((equate(rd1)==k)&&(A[k]==1))){
                                        InputPoss[j][k] = 1;
                     ...
                     //sum results from controller Bs processing
                    for(j=0; j<4; j++){
                        for(k=0; k<4; k++){
                            if(InputPoss[j][k]==1){
                                newB[k] = 1;
      ...
      //  }
        ...
        if((newA[0]==0 && newA[1]==0 && newA[2]==0 && newA[3]==0)||(newB[0]==0 && newB[1]==0 && newB[2]==0 && newB[3]==0)){
            printf("\nERROR:: Implication failed. No input value here can imply the provided output. \n");
